...
export function skullCloseupState(frame = null, plan = null) {
  ...
  const chestAmount = clamp(registrationOff
    ? 0
    : Math.max(
      probabilities.chest_dominant
        ?? (registration?.class === 'chest_dominant' ? registration.confidence : 0),
  ...
  const mixedAmount = clamp(registrationOff
    ...
    : Math.max(
      probabilities.mixed
        ?? (registration?.class === 'mixed' ? registration.confidence : 0),
      Number(plan?.inferredRegistration?.mixedField) || 0,
    ));
  const headAmount = clamp(registrationOff
    ? 0
    : Math.max(
      probabilities.head_dominant
        ?? (registration?.class === 'head_dominant' ? registration.confidence : 0),
...
export function mouthEmissionParticles({
  ...
  const out = [];
  const oralN = 96 + Math.round(drive * 64);
  const nasalN = Math.max(56, Math.round((70 + drive * 55) * Math.max(0.45, nasalShare)));
  const visualHz = phonated && frequencyHertz > 60 ? Math.max(1.6, Math.min(14, frequencyHertz / 48)) : 0;
...
export function exteriorCloseupParticles({
  ...
  const streams = [
    (t) => ({ x: -18 + Math.cos(t * Math.PI * 1.7 + 0.4) * 138, y: -36 + Math.sin(t * Math.PI * 1.7 + 0.4) * 156 }),
    (t) => ({ x: -70 + Math.cos(t * Math.PI * 1.4) * 40, y: -20 + Math.sin(t * Math.PI * 1.4) * 90 }),
    (t) => ({ x: 108 + t * 92, y: -18 + Math.sin(t * Math.PI * 5) * 16 - t * 8 }),
    (t) => ({ x: 118 + t * 88, y: 28 + Math.sin(t * Math.PI * 4 + 0.6) * 14 + t * 6 }),
    (t) => ({ x: 96 + t * 70, y: 8 + Math.sin(t * 9) * 10 }),
    (t) => ({ x: -86 + Math.sin(t * 7) * 18, y: 30 + t * 110 }),
  ];
...
function drawRegisterVoiceCloseup(ctx, L, state = {}) {
  ...
  if (chest > 0.04 || head > 0.04 || mixed > 0.04) {
    ...
    if (head > 0.04) {
      const vault = L.p(18, -78);
      const a = 0.22 + 0.44 * head;
      ctx.save();
      ctx.beginPath();
...
function drawOralCavity(ctx, L, formants, energy, timeMs, state) {
  ...
  ctx.strokeStyle = rgbaVoice(THROAT_CHAMBER_RGB, 0.55 + energy * 0.3);
  ctx.lineWidth = (1.8 + energy) * L.S;
  traceOralLumen(ctx, L);
  ctx.fill();
  ctx.stroke();
  ctx.fillStyle = `rgba(90, 42, 48, ${0.18 + fill})`;
  traceOralLumen(ctx, L);
  ctx.fill();
...
function drawLarynx(ctx, L, state, timeMs) {
  ...
  ctx.lineTo(L.p(22, 126).x, L.p(22, 126).y);
  ctx.lineTo(L.p(16, 154).x, L.p(16, 154).y);
  ctx.lineTo(L.p(-2, 154).x, L.p(-2, 154).y);
  ctx.closePath();
  ctx.fill();
  ctx.stroke();
...
function drawHeadSensationField(ctx, L, state, timeMs) {
  ...
  ctx.save();
  ctx.globalCompositeOperation = 'source-over';
  ctx.strokeStyle = rgbaVoice(SKULL_CHAMBER_RGB, alpha);
  ctx.lineWidth = (2.8 + amount * 2.6) * L.S;
  ctx.beginPath();
  ctx.moveTo(L.p(-36, -148).x, L.p(-36, -148).y);
  ctx.bezierCurveTo(L.p(24, -176).x, L.p(24, -176).y, L.p(102, -158).x, L.p(102, -158).y, L.p(112, -80).x, L.p(112, -80).y);
  ctx.stroke();
...
function drawLabels(ctx, L, state = {}) {
  const right = L.W - 18;
  label(ctx, 'skull chamber', right, Math.max(28, L.p(48, -150).y - 8), L.p(48, -150).x, L.p(48, -150).y, 'right');
  ...
  label(ctx, 'throat chamber', 16, L.H * 0.48, L.p(-10, 88).x, L.p(-10, 88).y, 'left');
